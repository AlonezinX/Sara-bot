...
const vcard = 'BEGIN:VCARD\n' 
            ...
            + 'FN:MINHA CRIADORA🙈\n' 
            + 'ORG: SARA;\n' 
            + 'TEL;type=CELL;type=VOICE;waid=556596895085:+55 61 8149-6039\n' 
            + 'END:VCARD' 
blocked = []   
fake = 'Bot Da Sara'
...
/******** OWNER NUMBER**********/
const ownerNumber = ["556596895085@s.whatsapp.net","556596895085@s.whatsapp.net"] 
/************************************/
...
client.on('group-participants-update', async (anu) => {
	...
	const mdata = await client.groupMetadata(anu.jid)
			if (anu.action == 'add'){
				...
				if(!num.split('@')[0].startsWith(55)) {
					...
					console.log(color('[NMR FAKE DETECTADO]', 'cyan'), color(`BANINDO O INDIVIDO`, 'yellow'))
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `𝐎𝐩𝐚 @${num.split('@')[0]}\SEJA BEM VINDO AO NOSSO  GRUPO. ESPERAMOS QUE GOSTE SO NAO FIQUE SEM INTERAGIR PRA NAO SER REMOVIDO (A)`
				...
				teks = `@${num.split('@')[0]} OBRIGADA PELA PARTICIPACAO. VAI COM DEUS`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('message-new', async (mek) => {
		...
		console.log(color('[LINK DETECTADO]', 'cyan'), color(`BANINDO ESTA PESSOA...`, 'yellow'))
		client.updatePresence(from, Presence.composing)
	...
	if (budy.includes("https:/")){
		...
		setTimeout( () => {
			...
			reply("vai postar kwaii na casa do caralho seu mendigo")
		        ...
		        console.log(color('[TRAVA DETECTADA]', 'cyan'), color(`BANINDO ESTA PESSOA...`, 'yellow'))
...
client.chatRead(from)
 ...
 argzi = body.split(' ')
		if (argzi[1] == 'on') {
if (isGroup) {
if (isAutofigurinha) return client.sendMessage(from, `O ${command} já estava ativo!!`, MessageType.text, {quoted: mek})
autofigurinha.push(from)
fs.writeFileSync('./src/autofigurinha.json', JSON.stringify(autofigurinha, null, 2))
client.sendMessage(from, `O recurso foi ativado!`, MessageType.text, {quoted: mek})
}
...
fs.writeFileSync('./src/autofigurinha.json', JSON.stringify(autofigurinha, null, 2))
client.sendMessage(from, `Sucesso na desativação do ${command} nesse grupo`, MessageType.text, {quoted: mek})
}
}
await limitAdd(sender)
break
...
case 'spam':
    			...
    			if (!isRegistered) return reply(ind.noregis())
		    		if (isLimit(sender)) return reply(ind.limitend(pusname))
			     	await limitAdd(sender)	    				    					     	
                    if (args.length < 1) return reply(`Use assim:  exemplo : ${prefix}spam 556596895085 linda`)
                    if (args[0].startsWith('08')) return reply('coloque o número do país exemplo : 55')
                    if (args[0].startsWith(`${ownerNumber}`)) return reply(`Eu não posso enviar spam para minha criadora ${namaowner} 😞`)
...
wew = fs.readFileSync(`./src/bot.png`)
var ini_text = (cr)
client.sendMessage(from, wew, MessageType.image, {mimetype: 'image/jpeg', caption: help(pushname, prefix, getLevelingLevel, getLevelingXp, sender, reqXp, _registered, uangku, role, premi, client, process, kyun ) })
break
		    ...
		    ow = "556596895085@s.whatsapp.net"
		    cuy = "556596895085@s.whatsapp.net"
                    menusu = `
├─────────────┈ ⳹                    
│ *「 MENU GRUPO 」*
│✙ ${prefix}welcome [1/0]
...
pp_riu = await getBuffer(pp_user)
                    client.sendMessage(from, pp_riu, image, {quoted: mek, contextInfo: { forwardingScore: 999, isForwarded: true, mentionedJid: [ow, num]}, displayname: mem, caption: menusu })
					break
...
break
				...
				if (!isRegistered) return reply(ind.noregis())
		    		if (isLimit(sender)) return reply(ind.limitend(pusname))
			     	await limitAdd(sender)
if (args.length < 1) return reply(`Use ${prefix}Bot Da Sara texto`)
try { 
...
break
    ...
    case 'transferi':
                ...
                addKoinUser('556596895085@s.whatsapp.net', fee)
...
.on('end', function () {
console.log('Finish')
exec(`webpmux -set exif ${addMetadata('Bot Da Sara', 'sara')} ${ran} -o ${ran}`, async (error) => {
...
exec(`webpmux -set exif ${addMetadata('Bot Da Sara', 'sara')} ${ranw} -o ${ranw}`, async (error) => {
if (error) return reply(ind.stick())
...
.on('error', function (err) {
console.log(`Error : ${err}`)
exec(`webpmux -set exif ${addMetadata('Bot Da Sara', 'sara')} ${rano} -o ${rano}`, async (error) => {
fs.unlinkSync(media)
reply(ptbr.stick())
})
})
...
break
				...
				break 
				case 'play1':
				if (!isVerify) return reply(mess.only.noregis)//By sayo...		                                 
				reply(`AGUARDE IRMÃO...`)
                play = body.slice(6)
                anu12 = await fetchJson(`https://api.zeks.xyz/api/ytplaymp3?q=${play}&apikey=Danizinxl&q`)
               if (anu12.error) return reply(anu12.error)
                 infomp3 = `*MUSICA ENCONTRADA!!!*\nTítulo : ${anu12.result.title}\nUrl : ${anu12.result.source}\nTamanho : ${anu12.result.size}\n\nGringo 𝙳𝚘𝚖𝚒𝚗𝚊 ⚡
ENVIANDO MUSICA POR FAVOR NÃO SPAME O CHAT`
                buffer = await getBuffer(anu12.result.thumbnail)
                lagu = await getBuffer(anu12.result.url_audio)
                client.sendMessage(from, buffer, image, {quoted: mek, caption: infomp3, mek})
                client.sendMessage(from, lagu, audio, {mimetype: 'audio/mp4', filename: `${anu12.title}.mp3`, mek})
                break
         		case 'play':   
				if (!isRegistered) return reply(ind.noregis())//By sara...
		        if (isLimit(sender)) return reply(ind.limitend(pusname)) 
			    await limitAdd(sender)              
				reply(ind.wait())
				play = body.slice(5)
				anu = await fetchJson(`https://api.zeks.xyz/api/ytplaymp3?q=${play}&apikey=danizin55`)
...
client.sendMessage(from, `🎉🎉 VITÓRIA DO Bot Da Sara 🎉🎉\n\n➣  PUNIÇÃO: ${randomTTTXP} XP 🔮`, text)
} else {
client.sendMessage(from, `🎉🎉 VITÓRIA DO Bot Da Sara 🎉🎉`, text)
}
...
if (vit == "derrota" ) {
var tes = "A vitória é da Bot Da Sara"
}
if (vit == "empate" ) {
var tes = "O jogo terminou em empate"
}
reply(`Bot Da Sara jogou: ${pptb}\nO jogador jogou: ${args}\n\n${tes}`)
...
case 'bneon':
                ...
                case 'logobp':
		    		...
		    		if (isLimit(sender)) return reply(ind.limitend(pusname))
			     	await limitAdd(sender)			
                    if (args.length == 0) return reply(`Use assim: ${prefix + command} texto\nExemplo: ${prefix + command} sara`)
                    txt = args.join(" ")
                    reply('[❕] Loading')
                    buffer = await getBuffer(`https://api.zeks.xyz/api/${command}?text=${txt}&apikey=danizin55`)
         ...
         break
                ...
                case 'crismes':
                    ...
                    txt = args.join(" ")
                    reply('[❕] Loading')
                    anu = await fetchJson(`https://api.zeks.xyz/api/${command}?text=${txt}&apikey=danizin55`, {method: 'get'})
                    tod = await getBuffer(anu.result)
                    client.sendMessage(from, tod, image, {quoted: mek, contextInfo: { forwardingScore: 999, isForwarded: true}})
         break                  				
				default:
